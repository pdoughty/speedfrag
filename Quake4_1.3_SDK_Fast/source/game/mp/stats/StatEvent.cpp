...
void rvStatKill::RegisterInGame( rvPlayerStat* stats ) {
	...
	statManager->GetLastClientStats( playerClientNum, ST_KILL, gameLocal.time - 2000, 2, (rvStat**)lastKills );
// PHIL BEGIN changed excellent and rampage to include POWERUP_HASTE and changed from 2000
	// check for excellent award
	if( !teamKill && lastKills[ 0 ] && lastKills[ 0 ]->GetTimeStamp() >= (gameLocal.time - 0) && lastKills[ 0 ]->victimClientNum != playerClientNum && victimClientNum != playerClientNum && player->PowerUpActive( POWERUP_HASTE ) ) {
		statManager->GiveInGameAward( IGA_EXCELLENT, playerClientNum );
// PHIL BEGIN
		idPlayer* player = (idPlayer*)gameLocal.entities[ playerClientNum ];
		gameLocal.mpGame.AddPlayerScore( player, 2 );
// PHILL END
	}
	...
	if( !teamKill && ( gibbed && victimClientNum != playerClientNum ) && 
		...
		( lastKills[ 1 ] && lastKills[ 1 ]->gibbed && lastKills[ 1 ]->victimClientNum != playerClientNum ) && player->PowerUpActive( POWERUP_HASTE ) ) {
			statManager->GiveInGameAward( IGA_RAMPAGE, playerClientNum );
// PHIL BEGIN
			idPlayer* player = (idPlayer*)gameLocal.entities[ playerClientNum ];
		gameLocal.mpGame.AddPlayerScore( player, 4 );
// PHIL END
	}
